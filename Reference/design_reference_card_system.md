...
| `cost` | CostExpr | Fungible resources **plus Intel Tokens** as a discrete-object cost category (18 cards: 10 CA, 4 PA, 4 ModReact) — "fungible only" was wrong. PS and presence tiers are not valid cost values; a marker shift that functions as a cost belongs in `success`/`fail`, since costs pay at submission regardless of outcome. **`cost=None` is a schema guarantee on ModActionCard** (folds into the host operation's packet, Art 03 §9.4.0.1 Step 4) **and ModBattleCard**; ModReactCard is the only Modifier subclass that charges — 27 of 93. **No cost magnitude rule is locked for any card type** — Principle 15 is a principle, not a formula, and costs were set by design judgement. Full reasoning: **Art 04c §3–§7**. Do not price new React cards against the convention in 04c §5; it describes 27 cards, it is not a rate (PM05 04c-01). |
...
value_rating: int | None   — **WHAT IT MEANS (Andy, S160 — PM02 L376, Art 04c): GROSS EFFECT DELIVERED.** A way to bucket cards by how much of the game they move; cards that affect more of it sit in higher tiers. It is **not** a net ledger — what the acting faction *pays* does not subtract from it, because `cost` is already its own field. Read that before touching the tier: the definition had never been written down anywhere, and at S160 that silence let a signed, net-of-payments measure get wired into `v_card_pair_uvm_cost`, which rated pay-to-impose cards (GUI.CA.7 buys a presence removal for 2 native) as floor-tier for paying a fair price. Reverted; units are unsigned magnitude. The signed axis still exists, deliberately kept OUT of the tier, in `v_card_value_to_acting` — see the header of `Database/view_card_pair_uvm_cost.sql` before rewiring anything. — 1–4 (widened from 1–3, S135/PM02 L259 — gives ModActionCard's 4-tier threshold_delta a distinct value per tier). **S143: moved to base Card() class** — no longer Modifier-subclass-only; all 251 CA/PA specs swept to `value_rating = None,  # scaffolded, not addressed`. **S145: definition locked and populated corpus-wide** — natural-break tiers on the UVM pricing model's `total_pair_cost` (schema_reference.md §6.7; methodology, governing caveat, and tier boundaries now canonically at Art 04c — derivation working history archived at `Retired/Whiteboard_Archive/cost_baseline_recommendations.md`). PM05 04-n178/n183 closed. **Inherited caveat: the underlying UVM base rates are calibrated off existing card costs, not playtested** — a `value_rating` is a self-consistency read against the current corpus, not an externally-validated power tier; treat it accordingly until real playtest data exists. None remaining = no computable `total_pair_cost` (blocked/TBD cards) or `GHO.CA.11` — not a design gap, revisit when each unblocks. **Two conventions coexist — this entry documents only one of them (clarified S157).** `value_rating` was populated by **two separate rules**, deliberately: **(a) CA/PA cards — the UVM tier scheme above** (PM02 **L284**, S145): natural-break tiers on `total_pair_cost` at **<3.0 → 1 · 3.0–4.99 → 2 · 5.0–6.99 → 3 · ≥7.0 → 4**, applied by a two-pass batch script to 162 of 164 CA/PA cards (2 done by hand, L283). **(b) Modifier cards — the earlier S132/S134 "mirrors magnitude" convention**, which L284 explicitly left untouched ("already carried a matching `value_rating`… no conflict"), resolving the 9 cards where the two rules disagreed in favour of the pricing tier. **176 of the 264 rated modifier cards are not in `v_card_pair_uvm_cost` at all** and were never tiered by cost — so a modifier card's rating reflects effect magnitude, not UVM cost, and should not be audited against the tier boundaries. **The ones that *are* in the view will show UVM drift, and that is expected, not a defect (Andy, S158).** Six do: four carry a rating the cost model disagrees with — GHO.MOD.7 (rated 2, cost 5.44), NET.MOD.9 (4, 2.00), NET.MOD.14 (3, 1.00), SYN.MOD.6 (1, 12.00) — and two are `None` but became computable when the table was rebuilt (GHO.MOD.1 10.72, STD.MOD.1 2.00). Recorded so they are not re-discovered as findings on a future sweep — `sync_card_db.sh` already excludes `%.MOD.%` from its drift query for the same reason. SYN.MOD.6 in particular was cited at S157 as evidence in a cost-model argument it was never subject to. **S158 — re-derived against the rebuilt cost model; the scheme now holds on every priceable CA/PA card (04-n228 closed).** The S157 verification above was computed against the *stale* `card_effect_component`, and **all four of its named drifters were corrected at S157** (PM02 L361: DIR.PA.8 4→2, GUI.CA.9 3→2, GUI.CA.4 3→2, DIR.CA.2 3→2) — they are absent from the S158 list because they were fixed, not because they were wrong. Re-checked against the rebuilt model: **three of those four corrections stand** — GUI.CA.9 (4.00), GUI.CA.4 (4.10) and DIR.CA.2 (4.13) have unchanged costs and the rebuilt model endorses their rating of 2. Only **DIR.PA.8** was corrected against bad input: the rebuild added its `on_accept` rows, moving its cost 3.00 → 6.00, so it moves again at S158 (2→3). Against the rebuilt table, 10 CA/PA cards drifted and were re-rated: STD.CA.14 2→1, GHO.CA.6 3→1, GHO.CA.10 3→1, SYN.PA.3 1→2, DIR.CA.4 1→2, STD.CA.7 1→2, DIR.PA.8 2→3, GUI.CA.8 2→3, GUI.CA.7 2→3, NET.PA.1 3→4. `bash tools/sync_card_db.sh` now reports ✓ no drift. **The S157 blanket caveat that every `value_rating` is provisional is lifted** — *that caveat only*, meaning "these numbers came from a stale substrate." **The Inherited caveat above is untouched and still governs**: the UVM base rates remain calibrated off existing card costs rather than playtest, so every rating is still a self-consistency read, not an externally-validated power tier. S158 fixed the substrate, not the calibration. One carve-out to the lift: the **five** bare-prose PAs (NET.PA.4/PA.5/**PA.6**, SYN.PA.4/PA.5) have no priceable effect rows, so their ratings are *unverifiable rather than wrong* and **hold** pending real MutationExpr (04-n218/n220) — they must not be re-rated to 1, and `sync_card_db.sh` reports them in a separate bucket rather than as drift. **`persistence_effect` counts toward `total_pair_cost` (Andy, S158)** — evidenced by `[persistence_effect]`-prefixed rows in the pre-rebuild backup and by the fact that the S145 ratings on DIR.PA.3/5/6/11 and DIR.MOD.9 reconcile only with inclusion; for six cards it is the sole priceable content. **Two cards' costs are floors, not values** — SYN.PA.3 and NET.PA.1 carry `has_boost`, so their tiers are floors at N=1 (NET.PA.1's is already at the tier-4 ceiling). DIR.CA.8 is rated but absent from the view entirely (no computable cost — related to `schema_cleanup_log` #27). **On the 8 `None` cards:** L284's "revisit per-card when each unblocks" is now due — 5 have become computable since S145 (GD-01 6.40, GHO.CA.11 26.67, GUI.CA.10 2.20, GUI.PA.10 9.25, SYN.MOD.1 1.00); 3 were then genuinely non-computable and correctly `None` (GHO.MOD.1, GUI.MOD.10, STD.MOD.1 "Overture") — **corrected S158: the rebuilt `card_effect_component` made two of those three computable** (GHO.MOD.1 10.72, STD.MOD.1 2.00); only GUI.MOD.10 is still non-computable. All three are Modifier cards, so they stay `None` under convention (b) rather than being tiered by cost — computable is not the same as in scope. GHO.CA.11 is **no longer** `id=TBD` as stated above — it is v1.0, though flagged for full reimagining, so its rating stays moot for a different reason. Tracked at `schema_cleanup_log.md` #60.
...
**S160 — four corrections to the entry above, and the drift query's key changed.** (1) The S158 re-rate list is partly superseded: **GHO.CA.6 and GHO.CA.10 are back to 3** (their S158 3→1 was computed against magnitudes the parser could not read — `IntelToken(...) * 3` and `.native * 2` are now read, 04-n235), and **GUI.CA.7 is 4, not 3** (gross 8.44). (2) **GHO.MOD.7's cost is 1.67, not 5.44** — the 04-n229 fix stopped billing a Redirect's two halves as two separate operations; the entry's "rated 2, cost 5.44" line is stale, and the same fix moved NET.MOD.10 and GD-01. (3) `sync_card_db.sh` **no longer excludes Modifier cards by `card_id NOT LIKE '%.MOD.%'`** — that string test leaked exactly one card (GD-01, a ModReactCard whose id carries no `.MOD.`) into the CA/PA tier audit; it now keys on `card_status.card_type <> 'MOD'`. (4) A **third** hold bucket exists alongside the bare-prose one: cards with mixed quantification across the ledger (04-n235) — currently just GUI.PA.6, whose 3-native gain counts in full while the structure it gives away is floored at 1 unit, so its 8.50 overstates it. Corpus otherwise reads **zero drift**.
