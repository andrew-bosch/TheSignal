...
| Battlefield Strength (no component) | Resolution | *Art 04 subject field: `BattlefieldStrength` — a contesting faction's Battlefield Strength total in a contested district (Art 03 §10.1.2). Registered S163 with `component_id = NULL`, like `InfluenceTier`: it is a derived quantity, not a physical piece. First and only user: GUI.MOD.10.* |
...
7b. **Add vs. Redirect — the discriminating test (S160).** `Add` is strictly *new element from supply*. `Redirect` is a cross-faction transfer: the same subject leaves one faction's holdings and enters another's, in one operation. Two traps:
   ...
   - **A cost paid is not automatically a transfer (S162, 04-n232).** Two cards can share the shape *acting pays, target gains, acting receives an instrument* and still split across the two Functions. The test is whether the **same element** arrives: `STD.CA.9 Fund` costs `Capital * 2` and delivers `faction(target).capital.add(2)` — same resource, same amount, ownership changed, so **Redirect**. `GUI.PA.2 Infrastructure Bond` costs `Capacity * 3` and delivers `faction(target).resource(native).add(2)` — Guild's Capacity is *consumed* and the **target's own** native is *created* from supply, so **Add**. (`faction(X).resource(native)` is X's native throughout the corpus.) The delivered instrument on either side — an Overture, a blank Accord form — is incidental; deciding the Function on which artifact feels more important is what produces mis-tags. Canonical statement: **Art 04b §5.1**.
   - **`GD-01`'s Redirect holds on every branch as of S162 (04-n230):** its structure removal is now gated on its structure placement actually executing, so the GR 8.2 skip no longer leaves it resolving as a bare `Remove`.
