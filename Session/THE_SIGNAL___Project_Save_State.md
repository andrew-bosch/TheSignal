...
**Last Updated:** 2026-09-20 — Session 162 Close

### Session 162 Summary (2026-09-20)

**A structural session. Art 00c's canonical conflict resolved by splitting the cost model into its own artifact, four of five S160 Art 04 gate items cleared, and a silent tooling defect found that had been weakening every drift check since the sync was built.**

**Art 04c — Card System Cost Model created and signed off at v2.0 (PM02 L378, L379); PM05 00c-03 CLOSED after being carried since S159.** Art 00c §5 had become the sole written home of the `value_rating` definition, the UVM pair-pricing methodology and the locked tier boundaries, while Art 00c's own header read *"Not canonical… Do not cite 00c for design decisions or ref files"* — and two documents already cited it as authority, one of them a Reference file the header forbids. Andy's ruling was a fourth resolution, not among the three logged: **split the model out entirely, and file it in the 04 series** rather than at 00-level, because it only ever prices card-system content. The artifact then widened from `value_rating` alone to card economics entire — **what cards charge** (§3 Principle 15 · §4 cost vocabulary · §5 cost by card type · §6 cross-resource costs · §7 Intel Tokens) and **how effects are priced** (§8 definition · §9 UVM + ⚠ governing caveat · §10 boundaries · §11 limits) — absorbing **Art 00c §8 Derived Cost Analysis as §12**, still gated on Art 04 sign-off. Art 00c is now a pure index at v0.7 with two pointer stubs, and its prohibition holds without exception. Five citations repointed; `Reference/design_reference_card_system.md` now contains zero references to Art 00c, so the ref-file citation is legitimate by construction rather than by exemption.

**Written down for the first time: no cost magnitude rule is locked for any card type.** Principle 15 is a principle, not a formula; the Balance checklist calls cost *"best-effort until Art 00c economics is built"*; §12 is that analysis and is blocked. The chain is judgement → average → tier, so §9's ⚠ caveat cannot be lifted until §12 exists — now stated in three places rather than none. **ModReactCard is the only Modifier subclass that charges resources** (27 of 93; ModActionCard folds into its host operation's packet, ModBattleCard is schema-locked because Art 03 §10.1.2 has no payment step at all — a procedural reason, explicitly not inherited). Its cost convention is recorded as **observed, explicitly not a rule** (PM05 04c-01), with its four departures named. **18 cards take an Intel Token as cost** (10 CA, 4 PA, 4 ModReact) — a discrete object with no defined exchange rate to fungible resources.

**Four of five S160 gate items CLOSED (PM02 L380), and two of their premises were wrong.** **04-n230** — GD-01's step 4 is now gated on step 3 executing, so the card never resolves as a bare Remove; the taxonomy settled it, since Art 04b defines Redirect as an ownership change and the ungated branch falsified GD-01's own tag. The cost interaction its `design_note` names was checked, not assumed: both generating cards restrict to districts where the holder has no structure, so the whiff requires the holder to build there after issuance. **04-n232 — closed as NO DEFECT**; STD.CA.9 moves the same resource (Redirect ✓) while GUI.PA.2 consumes Capacity and creates the *target's own* native (Add ✓). The discriminator — *does one and the same element change hands?* — is now governing in **Art 04b §5.1 (v2.7)** with that worked pair. **04-n231** — GD-01 added to §8; its framing question was answered by corpus practice, the other two Issued cards being already indexed. **04-n233** — three defects fixed, two worse than logged: GHO.PA.5 was the corpus's only presence placement lacking `faction=` *and* its only `game.add(PresenceToken, …)`, and the omission left `card_effect_component.target` reading `district` instead of `acting`. **04-n235 rehomed to 04c-01**, not closed — what remains of it is a pricing convention, which is a cost-model design ruling rather than a punch-list item.

**PM05 DB-50 — the sync extracted `card_effect_component` one cycle stale; found, fixed, verified.** `extract_card_effects.py` reads `card_body` **from the database**, but `sync_card_db.sh` ran it before reloading that table — so the `value_rating` drift guard evaluated the previous state of any card edited since the last run, and a single sync could report **✓ no drift** on a card whose cost had actually moved. Running the sync twice was an undocumented accidental workaround. Found empirically (a GHO.PA.5 edit appeared only after a second sync), fixed by reordering, and verified by reverting and re-applying the edit. The corpus reads ✓ no drift under the corrected ordering — **no current rating disagrees with freshly-extracted rows** — but whether any rating was originally set against stale input has not been re-derived.

**Opened:** **04c-01** (no cost magnitude rule for any card type; holds 04-n235's pricing convention) · **04-n237** (47 Art 04 §8 rows still assert the retired taxonomy exclusion — deliberately not swept) · **00c-04** (Art 00c's index is mis-provenanced and §3's "Round 1 Totals" table is *wrong*, not stale, because §4 carries no Resource Type rule) · **DB-49** (`card_status.cost_*` columns stale and hand-maintained — query `card_body` instead) · **DB-50**. Art 04 → v0.9.101; Art 04b → v2.7; Art 00c → v0.7. Derived docs synced: `Database/schema_reference.md` (sync ordering constraint + cost-column warning), `Reference/design_reference.md`, `design_reference_card_system.md`, `ref_taxonomy.md`, `ref_card_types.md`.

### Session 161 Summary (2026-09-11)

**A side session by design — no Art 04 work. The World Engine was chartered under governance, and a hand-run of its own checker found four canon defects in the creative corpus that two manual passes had missed.**

**World Engine chartered — PM01 WBS 4, eight deliverables, PM02 L377.** Andy proposed an AI-assisted canon governance, world-expansion and story-generation system, drafted with his work Copilot: canon held as entities and claims rather than documents, five truth layers, a ten-agent ecosystem, a continuous worldbuilding loop under human approval. Source captured verbatim at `Whiteboard/world_engine_proposal.md`. **His call: charter now, build later** — no canon, schema or generation work before Art 04 sign-off, with lev cleared for infrastructure groundwork only (4.02, briefed in `~/Airlock/claude-lev.md` with hard edges on scope). The canon-generating loop stays a real deliverable (4.05) — Andy wants the world expanded with a handle on consistency, gaps and drift — but paced to review capacity and surfaced as batched approval packets, rather than gated behind a queue that is not draining. **PM01 §6 clarified at Andy's direction:** the Tier 1 exclusion is agent-generated narrative *during a session*; agent-authored design artifacts under project-lead review are how this project is built, and the blanket wording contradicted the artifact set's own provenance. **AI-as-ARBITER logged as PM02 FD-07** — a far-future electronic-tier direction, cross-referenced to FD-01/FD-03/FD-05. A voice-activated ARBITER appliance over the same canon store is Andy's LLM-curriculum capstone with lev (4.08): personal learning, explicitly not expected at a play session.

**The first-pass assessment corrected the proposal's premises before they became a build — PM05 WE-01, eleven constraints.** Canon prose is **~36.5K words, not the 964K the repo suggests** (Art 00 16.1K, Art 01 4.6K, card `narrative`+`tagline` 9.7K, True State 6.1K; plus ~30K of Card Stories and 22K of explicitly non-canon `Creative/`). `04___Card_System.md` is a generated duplicate of its eight Part files — 332,435 words either way — so any ingestion that includes both enters every card twice; `Reference/*` are derived copies and excluded for the same reason. Vector storage is MariaDB's native `VECTOR`/HNSW, verified working on wakko's 11.8.9, not a second store. Truth-layer separation must be enforced in the retrieval query rather than by prompt instruction, with Art 00 §9.6 — ARBITER's threshold between what it processes and what it reveals, truthful but "not everything" — as the in-fiction warrant, so the design satisfies Pillar 6 instead of fighting it. And the approval queue, not generation capacity, is the binding constraint.

**Ten-role review of the `Creative/` corpus, run by hand as a dry run of the 4.04 checker** — all six vignettes, ~12 quotes and every `CANON_CANDIDATES.md` entry, checked against Art 00 v1.9, Art 01 v2.3, True State v1.0, `CREATIVE_BRIEF.md`, the card corpus, and git history where chronology was load-bearing. Findings: `Whiteboard/creative_role_review_s161.md`; actions: PM05 **CR-01** and **CR-02**. **Four of six vignettes go back to the writer, and two of those four are currently marked "No edits required" in the shortlist** — including the piece it calls the strongest in the set. "The Dry Run" gives the Chorus a 412-second recurring period, contradicting the one fact that has held for thirty-one years (Art 00 §6.1/§6.2, True State §1). "The Shack" renders reception as a directed energy flux that cooks soil fauna, which True State §1 denies. "The Calibration Problem" stages a Table session in year fourteen when The Table formed ≈ year 30 — the venue is the whole error, since Art 00 §14.2 has Ghost operatives naming the Holt Index before The Table adopted it. "The Ground Underneath" holds a year-seven filing for "eighteen" years when year seven to year 31 is twenty-four. Every fix is one or two sentences.

**The two findings that matter more than the rewrites are governance, and both were proven by chronology rather than inferred.** Art 00 §6.7 — signed off — absorbed the noodle-cart passage from a still-pending vignette seven weeks after that vignette was committed (2026-05-18 S18 → 2026-07-03 S134), **with the district changed** from a transit depot on the Infrastructure perimeter to the Commercial Strip; this resolves in canon's favour, so the vignette moves. And True State §11 has cited "Dr. Aris Thorne … per vignette" since 2026-05-27 (S44) under a header reading "Locked — Session 4", with no PM02 entry recording the decision — a missing decision record rather than corruption, since the document anticipates revision and §11 is its open-questions section. Consequently `Creative/README.md`'s "Canon That Has Been Incorporated: *Nothing yet*" is inaccurate, and the shortlist's placement claims are unreliable (it says Jae-won Seo is "Named in Art 01 §4"; §4 migrated to Art 02 at S90 and names nobody).

**Retired terminology runs through every submission — and the brief caused it.** "Sprawl" and "the Infrastructure" were renamed Baryo and The Mid at PM02 L141/L142, but `CREATIVE_BRIEF.md` itself carried "Sprawl" seven times until commit `4bc49e0` (2026-05-24), and the submissions are dated 05-16 and 05-18: the writers followed the brief they were given. The card corpus is already clean (Baryo 17, Sprawl 0; all 35 "Infrastructure" hits are the common noun or the card title *Infrastructure Bond*).

**Which roles earned their place — the finding that should drive the build order.** Every hard detection came from three roles: Archivist (terminology, unregistered institutions, name collisions), Historian (date arithmetic against fixed anchors) and Canon Diff (what canon already absorbed) — all lookup-and-compare work against a structured store, each finding arriving with a checkable citation. Editor required genuine judgment. Curator, Political Analyst, Cultural Anthropologist and Economist produced no detections at all, only ranking and gap-spotting — the work that stays with Andy, Gem or me.

**yakko's context envelope measured, correcting a claim made earlier in the same session.** Andy pointed out that 4-bit quantization leaves far more headroom than the 16K figure carried in memory. Measured on yakko: `gemma4:12b` is Q4_K_M, 11.9B, maximum context **262144**; VRAM runs 8.74 GB at 8K, 9.30 at 32K, 9.91 at 64K — roughly 8.6 GB of weights plus ~20 MB per 1K tokens against 16 GB, so **~128K context is comfortable** and the entire canon corpus fits in one local context. Retrieval is therefore an optimization for precision, speed and citation trails, not a workaround for a ceiling; what decides which roles run locally is verifiability, not context size. Corrected in WE-01 and in both memories.

**Also:** `~/Airlock` was proven to be an NFS4 mount served by brain — both nodes read and write the same bytes — disproving a memory that claimed those channels split-brain and need syncing. Memory corrected. lev's morning report (MariaDB dump auth fixed, ignored-file archiving, rclone pulling both to Drive) was ingested and the channel pruned live.

**Opened:** WE-01, CR-01, CR-02, FD-07, L377. **No artifact versions changed except PM01 → v1.7.** Art 00c-03 was deliberately left untouched — it is first on the S162 agenda and Andy's call.

### Session 160 Summary (2026-09-06)

**Two of Art 00c's three cost-model gaps closed, `value_rating` finally given a written definition, and the CA/PA corpus brought to zero drift. Art 04 → v0.9.100.**

**Add-vs-Redirect mis-tag sweep (gap #2).** Scoped wider than Art 00c asked, on the artifact's own text: 04b §5.1 defines Redirect as subsuming *"cross-faction resource movement = Transfer (Economy — Redirect)"*, so all 85 `Function=Add` cards were audited rather than the Territory subset. The discriminating test is that `faction(target)` serves two roles in this corpus — a type/currency qualifier (ARBITER supplies it; nobody loses anything) in most cards, a possessive in few. The borderline case was settled by precedent rather than judgement: GHO.MOD.7 and NET.MOD.10 are already tagged Redirect while implementing the transfer as two separate `arbiter.remove` + `arbiter.place` calls, so a transfer need not be a single atomic call to qualify. **GD-01 Grant Deed** was the sole mis-tag, retagged `Territory/Redirect/StructureBlock`; verified by grepping every effect field of all 85 for removal/move verbs — nine hits, of which six are self-directed and two are secondary.

**Toll/cut family — Andy ruled mechanics win.** `STD.MOD.104/105/116/117/128/129` run `faction(holder).native(faction=trigger.faction).add(1)`, which selects the rival's *currency* and debits nobody, while every one of them narrated a skim or a toll. Six Card Stories, six taglines, six `narrative` fields and STD.MOD.104's Design Rationale were rewritten to a capture-not-transfer framing with ring voice preserved, and three names renamed because they asserted a payer independently of the prose: **Toll Collected → Freight Booked**, **Informal Toll → Side Work**, **Cut of the Action → Rents Adjust**. STD.MOD.105's `narrative` was not a copy of its Card Story like the other five and still read *"The audit itself has a price, paid to whoever runs it"* — caught while editing.

**`target`-field semantics audit, then rewrite (gap #1).** `card_effect_component.target` was assigned by a four-rule first-match-wins regex cascade answering *"which entity is textually mentioned, by precedence"* rather than *"whose holdings change"*, and disagreed on **108 of 355 rows** — verified class by class, not sampled. All four classes fixed: hardcoded `faction(Syndicate)`-style names now resolve against the card's own faction (60 of 61 such rows name their own, checked by joining `card_status.faction`); the `district` test no longer pre-empts `faction=`; type qualifiers are blanked before the receiver search with the accessor preserved; and transfers get a destination rule. **The finding that reframed the gap: no view read `target` at all.** The problem was never inconsistency — the pricing path simply had no beneficiary axis.

**04-n229 — the Redirect double-count.** `v_card_pair_uvm_cost` billed a transfer's two halves as two priced operations, and took a different code path per effect category, so the same physical shape priced two different ways (GD-01 at 22.00 via the card's-own-Function path; GHO.MOD.7/NET.MOD.10 at 5.44 via sign-inference, never touching the Redirect rate that exists for it). An `xfer` CTE collapses matched opposing legs into one Redirect unit. Diffed all 205 priced cards: exactly 3 moved, the 3 predicted, and all three are ModReactCards — so no CA/PA rating shifted.

**`value_rating` defined — the session's most consequential outcome.** Andy: it means **gross effect delivered**, a way to bucket cards by how much of the game they move; `cost` is already its own field and records what is paid, so payment does not subtract. This had never been written down anywhere, and the omission was not harmless: earlier the same session the newly-corrected beneficiary axis was wired into `v_card_pair_uvm_cost`, making the tier a *net ledger*. That measure rated GUI.CA.7 — which buys the removal of a presence chip for 2 native — as floor-tier for paying a fair price, and moved NET.CA.6 3→1 and GUI.PA.10 4→1. All reverted; under gross those two compute 6.31 and 9.25, exactly their original tiers. The definition is now recorded as governing in **Art 00c §5** and in `design_reference_card_system.md`, so it cannot be re-derived from silence. The signed axis is kept, deliberately outside the tier, as `v_card_value_to_acting`, with both view headers explaining why it must not be rewired.

**Parser fixes (04-n235, partial).** The item's original framing — "variable-gain flooring", with `has_boost` detection as the lead — was wrong and was re-scoped: the `has_boost` gap is 2 rows on 1 card, and of the 5 cards it held only SYN.PA.1 was a variable gain. The real defect is **quantification asymmetry in either direction**, and the upward direction had been missed entirely (GUI.PA.6's gain counts in full while the structure it gives away is floored, so its 8.50 *overstates* it). Also characterised: of 101 NULL-magnitude rows across 84 cards, most price *correctly* at 1 unit, being a fixed single thing or not a quantity at all. `find_magnitude` now reads `.native * N`, `IntelToken(...) * N` and `min(N, ...)` — 7 magnitudes recovered, **SYN.CA.3 corrected 20.13 → 7.63** — and deliberately not `structure * 1` or `trigger.amount * 2`, where the multiplier applies to a counted thing. Re-rated GHO.CA.10 1→3, GHO.CA.6 1→3, GUI.CA.7 3→4. **Corpus now reads zero drift**, with only the five pre-existing bare-prose holds and one genuine mixed-quantification hold (GUI.PA.6).

**Two infrastructure defects, both found by the work rather than looked for.** `v_card_pair_uvm_cost` — the cost model's working view — **was defined by no file on disk**, existing only in the live DB, so a rebuild would have silently lost it; it and the new beneficiary view are now versioned under `Database/`. And the sync's drift guard excluded Modifier cards by `card_id NOT LIKE '%.MOD.%'`, a string test that leaked exactly one card — GD-01, a ModReactCard whose id carries no `.MOD.` — into the CA/PA tier audit; re-keyed to `card_status.card_type`.

**Wiki deploy hardened, after lev's post-mortem on the S159 close.** The queue executor had run `deploy_wiki.sh 2>&1 | head -50`; `rsync -avz` overran the pipe, SIGPIPE killed the script under `set -e` after `--delete` but before `mkdocs build`, and a `ps aux` check reading empty was taken as success. `trap '' PIPE` was tried first and **does not work** — ignoring SIGPIPE converts it to an EPIPE write error that `set -e` kills on anyway (measured). The fix that holds: all work runs with stdout redirected to a log, verified to survive `head -1`, ending in a `curl` check for 200. Session-close procedure updated to match.

**Opened, all gating:** 04-n230 (GD-01's GR 8.2 edge case), 04-n231 (GD-01 absent from the §8 index), 04-n232 (GUI.PA.2 vs STD.CA.9 tagged against each other's logic), 04-n233 (three passing-notice defects), 04-n235 (mixed quantification, now one card). **Closed:** 04-n229, 04-n234, 04-n236. Full detail: PM02 L370–L376.

### Session 159 Summary (2026-09-06)

**Both remaining Art 04 schema gates closed — `schema_cleanup_log` #64 and #65. Every schema gate on Art 04 is now clear; what stands between the artifact and a set-level sign-off is card-audit and content work only.**

**#64 — the PositionalWager re-derivation.** All 72 non-React `Automatic` card bodies were read by hand against the §6.3 definition. The item warned that a query could not complete the sweep, and the reason is now documented rather than asserted: GUI.CA.2's `success` is an unconditional two-line resource add with no forward reference and no conditional — textbook deterministic in the body — while its Design Rationale reads *"betting a Beat 2 slot that this faction will execute STD.CA.2 this Quarter."* The wager exists only in prose. Every card was therefore read with its Design Rationale and Card Story alongside its `Card()` block, and `tagline`/`narrative`/`design_note`/`arbiter_note` treated as mechanical evidence.

Population was derived two ways rather than taken from the item: 39 CovertOperation + 33 PublicAct = 72, with `card_status.card_type` and `v_card_body.type` agreeing and zero mismatch; all 92 `Automatic` MOD cards are literally `type=ModReactCard`. Enum check clean — no `Probabilistic` mislabel among `Automatic` cards, none among d100. An empirical profile of the 13 pre-existing instances was derived, not assumed: every one is `beat = 2`, CovertOperation, `persistence = Immediate`. Beat 2 is the only beat that precedes others inside the Quarter's resolution sequence, which is what grounds the first ruling below.

**Two boundary rulings (Andy), both now specification in §6.3.** (1) A contingency carried by `persistence`/`persistence_effect`/`game.world_condition`, or by a delivered instrument such as a Grant Deed, leaves the card **Transactional** — its own resolution was deterministic and the persistence fields already express what follows. This holds 13 cards that would otherwise have flipped (GUI.CA.10, SYN.CA.8, GUI.PA.3, GUI.PA.8, GHO.PA.2, DIR.PA.1/3/5/6/7/11, SYN.PA.2/PA.5); without it the label would swallow every reactive board condition in the corpus and stop discriminating. (2) The unrevealed slate **need not belong to a later beat**, only to be unrevealed at commitment — §6.3 names the discriminator as unknown information at commitment and phrased "a Beat 2 card modifying a Beat 4 op" as an example. That example was being read as a boundary, which is precisely why ruling 2 was needed, and it has been removed.

**Eight recategorisations applied, corpus 13 → 21:** GHO.CA.1, DIR.CA.1, GHO.CA.3, DIR.CA.3, GHO.CA.5, STD.CA.12, NET.CA.1, NET.CA.4 — six of them new this session. DIR.CA.1 turned out to be mislabelled against its own Design Rationale, which already read "Beat 2 Automatic positional wager" and repeated it in two checklist rows. All 21 instances are CovertOperation; no PublicAct can qualify, which falls out of beat ordering rather than being a design choice. Design Pass was reset on all 8 per 04-n225's standing materiality test (corpus now 18 at `design_pass = 0`).

**GHO.CA.1 reversed a logged decision rather than filling a blank.** Item #41 had ruled it `Transactional` as a *"deterministic declare-then-verify check"* — reasoning that does not survive, since "deterministic once resolved" is part of the `PositionalWager` definition itself. Its supporting appeal to the card's own perspective line (*"We are not predicting"*) also fails: that is Ghost's voice asserting competence, not a claim about the resolution mechanism, and it reads correctly against a wager. The Design Rationale was rewritten and approved before the field changed, and the `schema_cleanup_log #41` citation inside the card prose was dropped per item #54. Andy added a mechanical clarification to the same card: the stolen operation card **returns to its owner's Dispatch Case once it has resolved** — Ghost takes that Month's execution, not the card, and gains no lasting capability. Added as an explicit step 5 in `arbiter_note` and to `design_note`; the old "their case returns empty" line was corrected to match.

**Flagged, not resolved — GUI.CA.9 Works Guarantee may be an over-inclusion in the original 13.** #64 grouped it with GUI.CA.1/2/6 as wagering semantically on a rival's later action, but CA.9 operates on **Guild's own** named Beat 3 CA (`restriction = target_ca in game.dispatch.guild.beat3`), with district B checked at Beat 0 against board state already visible at §9.1 — so there may be no unknown at commitment beyond the residual risk every card carries. The enumeration was never derived, which is the whole reason #64 existed; it can be wrong in this direction as easily as the other. Tracked, not gating.

**#65 — the §6 spec-hygiene sweep.** §6 now carries **zero** `schema_cleanup_log`, PM05, PM02 or session citations and **zero** uses of "confirmed", verified in both the Part file and the regenerated monolith. Counts were re-derived before working rather than carried from the S157 scan, and they differed: 13 `schema_cleanup_log` · 10 PM05 · **6** PM02 `L###` where the scan had said 3 (it missed the bare `L219`/`L280`/`L174` forms) · 3 session refs · 17 "confirmed" · 14 "Displayed" TBDs. Removed as corpus analysis rather than specification: the §6.0 research-provenance note, the Acquisition-axis Issued-card inventory, `persistence`'s example card list, `acquisition`'s "all current Issued cards happen to be ModReactCard", `generating_card`'s instance examples, the three "Confirmed via [card list]" trailers under MutationExpr / `game.active_permanents` / `district.where`, and `ring_constraint`'s "semantics under review" note (still tracked at 04-n161). The `cost`-is-`None` italic paragraph was rewritten to state why each subclass takes `None` without recounting how the two decisions were reached. Artifact cross-references (Art 03, Art 04b, Art 02/DB:, GR, Design Pillar, P##) were kept throughout — those are spec pointers, not provenance. The 14 "Displayed" TBDs stay, now with a line under the Data Dictionary naming §7 and Art 09 as the gate, so the column reads as open specification rather than an unexplained blank.

**Four factual defects surfaced by the sweep, all outside the item's original scope, all fixed.** (1) `beat: int  # 1–5` was wrong on both readings — Art 03 §9.4 runs Beats 0–4 and the corpus uses only 2/3/4 (23/46/47 cards, `None` on modifiers); Andy's call was to enumerate the real values, and §6.2's Data Dictionary row was aligned to match. (2) §6.1 contradicted §6.2 on `value_rating`, still reading "CA/PA meaning not yet defined… pending whole-set cost-derivation analysis" against §6.2's "assigned corpus-wide, locked S145." (3) §6.2's "`None` = unassigned (blocked/TBD cards only)" was false — the three `None` cards (STD.MOD.1, GUI.MOD.10, GHO.MOD.1) are unblocked ModReactCards held `None` by convention. (4) `persistence` named DIR.PA.1 as a Permanent example when it is `Seasonal`; resolved by that list's removal.

**Art 00c §5 updated in the same pass (Andy's call), Art 00c → v0.6.** Art 04 §6.2 points there for `value_rating`'s methodology, caveat and tier boundaries, and it still read "locked S145" with pre-S158 tier counts. It now records S158's closure and the ten re-ratings, states the model's CA/PA-only scope and the MOD/React magnitude-convention exemption, carries the five bare-prose PA carve-out, and its tier-boundary counts were re-derived from `v_card_pair_uvm_cost` — 114/49/23/19 across 205 priced cards, previously 107/48/26/19.

**One conflict flagged and deliberately left for Andy.** Art 04 §6.2 — a specification — cites Art 00c §5 as the authority for `value_rating`'s derivation, while Art 00c's own header reads "⚠️ Future Analysis Stub — Not Canonical" and instructs *"do not cite 00c for design decisions or ref files."* Fixing §5's content does not fix that. The resolutions are to move the methodology into Art 04 §6 or a `Reference/` file, or to lift the non-canonical status for that one section — both change an artifact's canonical status, so neither belonged inside a hygiene sweep.

**Derived-doc sync.** `Reference/design_reference_card_system.md` was badly stale on this field: it still described `resolution_type` as *"str, not enum"* and told authors *"do not use `\"Positional wager\"` unless matching an established pattern"* — language predating the S147 enum conversion entirely — and it repeated the same wrong `beat` range. Prose rule and field table both rewritten against the amended §6.3. `Database/schema_reference.md` carried a claim that `design_pass`/`issues_resolved`/`signed_off` "read 0/blank corpus-wide," which was wrong when written; replaced with the re-derived live distribution (`design_pass` 368/18, `issues_resolved` 78/308, `signed_off` 0/386, `structure_pass` 380/6) plus a note that `design_pass` resets are a live mechanic under 04-n225, not a one-time event.

**Artifacts:** Art 04 → **v0.9.99** · Art 00c → **v0.6**. Monolith regenerated and both DB mirrors reloaded; `sync_card_db.sh` reads ✓ no drift. Per-card evidence for all 72 cards retained at `Whiteboard/pw_rederivation_s159.md` — the rulings and the eight changes are in the logs, but the individual calls, including the 64 held `Transactional`, live only there.

### Session 158 Summary (2026-08-24)

**PM05 04-n228 CLOSED — the last Art 04 cost-model gate. `value_rating` is no longer provisional.** Andy ruled that `persistence_effect` counts toward `total_pair_cost`. The finding is that the question was never actually open: the pre-S157 backup of `card_effect_component` contains five rows whose `raw_expr` opens with a literal `[persistence_effect]` prefix (DIR.PA.3/5/6/11, DIR.MOD.9, all `board_condition`/NULL — the same convention the rebuilt extractor uses, different punctuation), and the existing S145 ratings on those cards reconcile *only* with inclusion: excluding it drops their computed costs to tiers 2/1/1/1 against ratings of 4/3/3/2. For six further cards the persistence effect is the entire priceable content, so excluding it leaves nothing to price at all.

**Method.** The `v_card_pair_uvm_cost` inner logic was rebuilt as a CTE and run in three regimes — include / exclude / the old `board_condition`+NULL encoding. Include-mode reproduces the live view row-for-row across all 199 shared cards, so the replication is faithful. Old-vs-new encoding is **tier-neutral corpus-wide** (only DIR.PA.11 4.56→4.28 and DIR.PA.5 3.44→4.00 move at all, neither across a boundary), which means the S157 re-encoding changed nothing that reaches a card face. Only inclusion-vs-exclusion ever mattered.

**Three S157 claims did not survive re-derivation — and the failure mode is the one S157 itself kept hitting: a claim about the corpus asserted from a derived artifact rather than re-derived from source.** (1) *"That single choice drives SYN.MOD.6 from tier 1 to tier 4"* — SYN.MOD.6 computes tier 4 in **all three** regimes (12.00 / 7.00 / 8.00); its tier is driven by `arbiter.modify(..., boost, delta=+20)` and the 2-Capital escrow, not by persistence. The claim fails a second time independently: SYN.MOD.6 is a MOD/React card, so it rates on the S132/S134 magnitude convention and was never UVM-tiered at all. The one argument that made the question look blocking rested on the single card it could not apply to. The choice in fact gated **one** card, SYN.PA.3. (2) *"The four bare-prose PAs"* — there are **five**. NET.PA.6 has no priceable effect rows either and its rating of 1 is unverifiable identically; it stayed invisible only because 1 happens to equal what the tier scheme spuriously returns at cost 0.00. 04-n218 had already scoped NET.PA.4/PA.5/PA.6 together — the punch list was right and the cleanup-log enumeration was the incomplete one. (3) *"14 CA/PA cards report as drifted"* — independent re-derivation returns **16**, of which 6 are MOD/React and correctly out of scope. S157's enumeration of the **10** re-derivable cards was, however, exactly right, card for card and direction for direction.

**All 10 re-ratings applied:** STD.CA.14 2→1, GHO.CA.6 3→1, GHO.CA.10 3→1, SYN.PA.3 1→2, DIR.CA.4 1→2, STD.CA.7 1→2, DIR.PA.8 2→3, GUI.CA.8 2→3, GUI.CA.7 2→3, NET.PA.1 3→4 — each spot-verified by hand against `uvm_pair_assumptions` (zero rate-table fallbacks, every rate `validated`). Two carry `has_boost`, so their costs are **floors at N=1**, not values: SYN.PA.3's tier 2 could rate higher once N is known; NET.PA.1's floor is already at the tier-4 ceiling and therefore safe.

**Rulings (Andy).** `persistence_effect` counts toward cost. MOD/React cards drifting against the UVM tiers are **expected, not defects** — the six that do (GHO.MOD.1, GHO.MOD.7, NET.MOD.9, NET.MOD.14, STD.MOD.1, SYN.MOD.6) are now recorded by name in `design_reference_card_system.md` so a later sweep does not re-discover them as findings. All five bare-prose PAs **hold** their current ratings — unverifiable rather than wrong — pending real MutationExpr via 04-n218/n220.

**Tooling and drift-guard fix.** `sync_card_db.sh` counted the bare-prose cards as drift, which would have become a permanent false positive masking genuine drift. The check now reports them in a separate "unpriceable — bare prose" bucket; that split is what surfaced NET.PA.6. The drift check reads **✓ no drift**. A reverse pointer was added to PM05 04-n218 and 04-n220, neither of which mentioned `value_rating`: converting their prose to MutationExpr makes those ratings computable for the first time and they must be re-derived at that moment — without the pointer, 04-n228's carve-out was a dangling promise and exactly the silent-drift class this work existed to close.

**Three corrections to my own S158 work, caught before they landed.** A backup-parsing regex silently dropped 78 of 358 rows because it could not match negative magnitudes — re-parsed with a quote-aware tokenizer and validated against `AUTO_INCREMENT`. A Reference edit claimed S157's four named drifters were "artefacts of a stale snapshot" when PM02 L361 records that all four were *corrected* at S157; re-checked, three of those corrections stand against the rebuilt model (GUI.CA.9, GUI.CA.4, DIR.CA.2 — unchanged costs, rating 2 endorsed) and only DIR.PA.8 was corrected against bad input, its cost moving 3.00→6.00 once its `on_accept` rows were added. And the same paragraph still claimed three cards were "genuinely non-computable" when the rebuild had made GHO.MOD.1 and STD.MOD.1 computable — they stay `None` under convention (b), but computable is not the same as in scope.

**Consequence.** The S157 blanket caveat that every `value_rating` is provisional is **lifted** — that caveat only. The governing UVM caveat (base rates calibrated off existing card costs, not playtested) is untouched and still stands: S158 fixed the substrate, not the calibration. Art 04 → **v0.9.97**. PM02 L367; full detail in `schema_cleanup_log.md` #66, S158 addendum.

**Next up:** `schema_cleanup_log` #64 — the PositionalWager re-derivation, a 72-card hand read that wants a clear session.
...
**Sequencing decision (PM02 L352):** PM05 09-16 steps 4–5 (faction-level + cross-faction card set re-audit) confirmed primary, ahead of the 95-card Art 03 procedure-gap list (04-n221) — closes the open question left at S154 close. Any new procedural/redesign items steps 4–5 surface merge into 04-n221 as found, rather than becoming separate PM05 items.

**First concrete step on Andy's "DB-queryable Art 04" goal (PM02 L353): new `card_checklist` table live in `the_signal_db`.** Mirrors every card's Design Checklist rows (category/verdict/note/artifact_ref) — 6,970 rows across 383 cards, joins cleanly to `card_status`. Scoped deliberately narrow (checklist verdicts only, not a full `Card()`-field migration) — Andy's own framing of the choice ("would a queryable table help the audit, or is the .md easier — your call") resolved to: mirror exactly what steps 4–5's cross-faction slicing needs (per-category ⚠ counts by faction, etc.), leave `.md` as source of truth. Extraction script `tools/extract_card_checklist.py` built via agy delegation (schema + parse rules mine, bulk extraction agy's), then hardened directly after catching a real bug in agy's first pass: ~20 cards' markdown headings omit the real card_id (bare faction-name headings, e.g. `### STANDARD — DISINFORMATION CAMPAIGN` instead of `### STD.CA.13 — ...`), and trusting heading text for `card_id` silently collided multiple distinct cards onto one fake ID — caught via the table's `UNIQUE(card_id, category)` constraint failing on load, fixed by resolving `card_id` from each card's own `Card(...)` block instead (matching what `audit_card_corpus.py` already relied on). Also correctly excluded the pre-existing Backdate/Field Verification gap (both share placeholder `id="Ghost-ext-TBD"`, no real `card_id=` field — same 2-card gap already excluded from `card_status`). 3 unescaped literal `\|` characters fixed in source markdown (`STD.CA.10`, `GHO.CA.5`, `NET.CA.7`) as a side effect of building the parser.

**Heading/SOT cleanup + Backdate/Field Verification relocated (PM02 L354).** Andy's follow-up question — since the checklist script now reads `card_id` from the `Card()` block rather than the heading, do we still need to fix the headings themselves? Yes: the underlying data was never wrong, but ~18 real cards' markdown headings displayed only a bare faction name instead of their ID, inconsistent with the rest of the corpus. Fixed all 18 (Standard STD.CA.13–16; Guild GUI.CA.6; Ghost GHO.CA.6/7/8/9/10/11/12; Directorate DIR.CA.5/PA.3/PA.6; Network NET.CA.6/PA.3; Syndicate SYN.CA.6/8/9) plus their TOC rows/anchors — 2 of which (GHO.CA.6/11) had also been showing `—` for their ID in the TOC despite having real `Card()` IDs, fixed the same pass. Anchor convention settled empirically against existing corpus anchors (`dirpa4--regulatory-downgrade-stub` etc.): periods/slashes vanish, em-dash-with-spaces becomes `--`. Separately, per Andy's direction, Backdate and Field Verification (the two Ghost fossil cards permanently 🚫 BLOCKED since S107/04-n103, GR 7.2b, sharing the placeholder ID) don't belong in the live §7 corpus at all — moved verbatim (content unchanged) to a new **Part1_Core.md §15 "Deferred Design — Blocked Cards"** section, Ghost's own §7 TOC repointed to it, PM05 04-n205 updated with the new location. Monolith `04___Card_System.md` regenerated; `card_checklist` re-extracted to confirm zero data drift (still 6,970 rows, zero heading-vs-card_id overrides now needed, zero collisions).

**Steps 4–5 scope refined with Andy, execution approach settled.** Format: same as the old S119–128 per-faction + cross-faction passes (deck feel, doctrine coherence, layer coverage, win-path support, §9.2 floor/ceiling economics vs Art 04 §5a), now covering each faction's full CA+PA+MOD set for the first time — the old pass only ever saw CA/PA, predates the 232-card modifier corpus entirely. `Whiteboard/card_analysis_summary_S119-128.md` is the comparison baseline only, not ground truth to assume — re-derive fresh, then check against it. **New dimension:** explicitly identify (a) *implied* strategies — what §5a/doctrine claims a faction should be able to do, rated thin/thick against actual card support (extends the old "win-path support" rating into a first-class axis); (b) *leverageable* strategies — plays the current card set enables whether or not §5a ever named them. Step 4 = both within one faction's own toolkit; step 5 = cross-faction synergies/counters that only show up in comparison (e.g. the old Network-disable-vs-Guild-no-covert-ops interaction). This folds in what the old cross-faction synthesis handled ad hoc via a separate card-ideas doc (4 emergent patterns, closed no-op S128) directly into the formal steps 4–5 process. **Step 4 order confirmed:** same sequence as the old audit — STD+DIR → GHO → GUI → NET → SYN, checkpoint with Andy after Directorate before replicating across the remaining four. **Execution approach:** run as the main loop directly at Opus or Fable strength (not subagents) — this is collaborative design work done in-conversation with checkpoints, not a report to generate and hand back; a subagent can't checkpoint with Andy mid-audit, and the S141 finding (full reference context + sequential faction-by-faction review surfaces more real findings than batching) argues for careful sequential work over parallel fan-out.

Regression suite confirmed clean throughout: monolith regenerated after every Part-file edit, `card_checklist` extraction re-run and re-verified after the heading/appendix changes (6,970 rows, 0 overrides, 0 collisions).
