...
def main():
    ...
    lines = []
    # Reordered header
    lines.append("| Ring | Address | Origin District | Adjacent Ring | Adjacent Address | Adjacent District | Allow Ingress | Allow Egress |")
    lines.append("|------|---------|----------------|--------------|-----------------|-------------------|--------------|-------------|")
    
    for r in rows:
        lines.append(f"| Ring {r['o_ring']} | {r['o_addr']} | {r['o_name']} | Ring {r['n_ring']} | {r['n_addr']} | {r['n_name']} | TRUE | TRUE |")
    ...
    for i, line in enumerate(md_lines):
        if line.startswith("| Ring | Address | Origin District |"):
            start_idx = i
    ...
    if start_idx != -1 and end_idx != -1:
        ...
        print(f"Successfully reordered columns and updated table in {file_path}")
    else:
