...
Maximum 1 Modifier Card per action submitted — applies to ModActionCard/ModReactCard (dispatched with or reacting to a submitted action). **Does not apply to ModBattleCard:** Battlefield Strength commit (§10.1.2) is not an action submission, and Art 04 §11.4 (the source of this cap) is itself flagged non-canonical/sketch-stage — confirmed S132, no quantity cap on Battlefield Modifier Cards. Excluded from taxonomy.
