...
- **ModBattleCard** — committed during Art 03 §10.1.2 (Calculate and Declare Totals) only, as part of Battlefield Strength resolution. Entirely separate from React cards; not playable outside that window. `effect: ModBattleExpr` — `direction: Boost | Hinder` + explicit `target: Faction` (any contesting faction, chosen at commit — need not be the playing faction) + `magnitude: int` (redesigned S132, PM02 L242; supersedes the earlier `Self | Opponent` framing, see design_reference_card_system.md). Any faction — contesting or not — may commit a card into an active contest, naming which contestant it affects; commit is face-down in front of the named target, revealed simultaneously with all other commitments before the d10 roll. Discarded on use regardless of outcome — may not be replayed in the same contest or any subsequent district contest that Quarter. **Faction-set stub pass complete S132: 20 cards, all 5 factions, 2 Boost (+1/+2) + 2 Hinder (−1/−2) each, `cost = None` uniformly, magnitude/`value_rating` playtest-flagged (04-n94) — see `modifier_card_ideas.md` for the full per-faction breakdown and voice notes.** Ring-set content still pending, gated on 04-29 (ring-voice narrative gap).
...
**Ring Modifier Cards** — drawn from shared ring decks on the game board (Baryo/Mid/Core; no Chorus Node deck). Draw requires: 1+ Structure Block AND Established+ in at least 1 district in that ring. Back: ring color. **04-n161 ✅ closed S134:** rather than one default, `ring_constraint` is set per card into two parallel sets per ring — Portable (`None`, travels with the holder) and Ring-Locked (usable only in Battlefield Strength for that ring's districts) — decided by whether the card concept is location-anchored or genuinely portable. First content: STD.MOD.2–25 (Art 04 §7), 8 ModBattleCard stubs per ring (4 Portable + 4 Ring-Locked), voiced from Art 00 §6.7 "Ring Character."
