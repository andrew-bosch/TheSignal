...
**Ring Modifier Cards** — drawn from shared ring decks on the game board (Ring 3/2/1; no Chorus Node deck). Draw requires: 1+ Structure Block AND Established+ in at least 1 district in that ring. Back: ring color. ⚠ "Ring constraint applies to all users regardless of holder (Principle 23)" reopened as an unresolved semantics question S133 (PM05 04-n161, open) — Andy questioned whether Ring cards should even default to a ring-locked constraint at all ("if you got equipment from Baryo, what is stopping you from leveraging it in a different location?"). Don't treat this line as settled until 04-n161 closes.
