...
| EC-xx | Event Card / Situation Report | Dual-card | Broadcast Card (public) + Event Card (ARBITER-only). Not yet designed. |
...
Operational DB table tracking real-time physical location of every in-play component. Formerly `live_state` (rename pending agy DB-11).
...
- O-xx, EC-xx, CC-xx, ER-xx: not yet designed (blocked on source artifact)
- `component_positions` rename: pending agy DB-11
