...
| **Governing Rule 10.1** HARD | No card may compel disclosure. A Reveal effect creates a stake (consequence for revealing or withholding) — the decision remains the player's. ARBITER-reveal is outside 10.1 (ARBITER discloses from its own domain; portrait track is the only prohibited ARBITER reveal target per L225). | All Reveal cards; distinguish faction-compelled vs. ARBITER-sourced reveal |
