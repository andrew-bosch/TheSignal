...
**Art 04 `.md` mirror tables (rebuilt from source, never edited directly — S155–S157):**
`card_body` (17,995 rows — EAV mirror of every `Card()` field, added S156; count as of S163), `card_restriction_clause` (117 rows — `restriction` decomposed to subject/op/value, S156), `card_checklist` (6,970 rows, S155), `card_effect_component` (365 rows — outcome fields decomposed into priced units; extractor added S157).

**⚠ These four are derived, not authored.** `.md` is SOT. Rebuild all of them plus the Art 04 monolith with **one command**: `bash tools/sync_card_db.sh` (`--check` reports drift without writing). Never hand-edit them and never `UPDATE` them to fix a value — fix the `.md` and re-sync, or the change is lost on the next rebuild. `card_status` is deliberately **not** in this set: it holds review state (`design_pass`, `issues_resolved`), not extracted content, and must be updated deliberately.

**⚠ Ordering constraint inside the sync — fixed S162, PM05 DB-50.** `extract_card_effects.py` reads `card_body` **from the database**, not from the `.md` or the load file. Until S162 the sync ran it at step [4/5], *before* step [5/5] reloaded `card_body` — so `card_effect_component` was always one sync behind the corpus, and every drift guard below evaluated the **previous** state of any card edited since the last run. A single sync after a card edit could report **✓ no drift** on a card whose cost had actually moved; running the sync twice was an undocumented accidental workaround. The extractor call now sits inside step [5/5], after `card_body_load.sql` lands and before `card_effect_load.sql` is applied. **If that call is ever moved again, this is the constraint it must respect.** The corpus reads no drift under the corrected ordering, but whether any current `value_rating` was originally set against stale input has not been re-derived.

**⚠ `extract_card_body.py` silently absorbs malformed input — it reports a smaller number, not an error (PM05 DB-51, S163).** The extractor splits a `Card(...)` call into fields at **depth-0 commas**. A field written *inside* a nested expression — e.g. an `on_discard = None,` line landing between `on_decline = (` and its closing paren — is therefore not a field at all: it is folded into the parent field's `raw_value` and produces **no `card_body` row**. Nothing rejects it, nothing warns, and the only symptom is a row count lower than the corpus should yield. This was found at S163 when a scaffolding sweep inserted 66 fields at paren depth > 1 across 25 cards and `card_body` did not move; the reading was initially misdiagnosed as a sync-reporting lag (a DB-50 relative) before the real cause was traced. **Practical consequence: after any bulk edit to `Card()` blocks, do not treat an unchanged or unexpectedly small `card_body` count as reassurance** — check that inserted fields actually appear as rows, and check a sample card's field count against its siblings. A generated field is only real if it sits at depth 0.

**⚠ `card_status.cost_*` columns are stale — do not query them (PM05 DB-49, S162).** `cost_type`, `cost_variable`, `cost_primary_amount`, `cost_native_count` and `uses_intel_token` are hand-maintained and nothing checks them: `cost_native_count` is non-zero on only 19 of 386 rows, `uses_intel_token` is set on 16 where 18 cards carry an Intel Token in their actual cost, and at least one `cost_type` disagrees with the card body (SYN.MOD.1 reads `free` while charging `Findings * 1 + IntelToken(...)`). **For anything cost-related, query `card_body` where `field_name='cost'`** — that is the sync-regenerated mirror and is authoritative.

**What the sync reports after writing (read these, they are the drift guards):** a `value_rating` drift check against the L284 tier scheme (CA/PA only — `%.MOD.%` is excluded, because Modifier cards rate on the S132/S134 magnitude convention and auditing them against cost boundaries produces false positives); a **separate** "unpriceable — bare prose" bucket (**added S158**) listing cards with no rows in `card_effect_component`, whose ratings are *held* pending 04-n218/n220 and must not be re-rated — folding these into the drift list made a permanent false positive that would mask real drift, and splitting them out is what surfaced NET.PA.6 as a fifth such card; and a taxonomy↔pricing check that every in-use Subject has a base rate.
...
**card_status — 4-state review progression:** `structure_pass` (has the 4 structural blocks — set by `tools/card_scaffolder.py` or by writing a card in full directly), `design_pass` (checklist content actually reviewed, not just scaffolded), `issues_resolved`, `signed_off` (Andy's explicit approval). A card can be `structure_pass=1, design_pass=0` — scaffolded but unreviewed; that state has no MD equivalent besides the checklist rows still reading `⚠` throughout. Keep this column in sync whenever `tools/card_scaffolder.py` runs, same as the existing sync discipline for `design_pass`/`signed_off`. **Review-state distribution is live, not blank** — re-derived S159: `design_pass` 368/18, `issues_resolved` 78/308, `signed_off` 0/386, `structure_pass` 380/6. (An earlier note here claimed all three read 0/blank corpus-wide; that was wrong when written and is corrected rather than updated.) `signed_off` is genuinely 0 everywhere — no card has Andy's approval yet. **`design_pass` resets are a live mechanic, not a one-time event:** per PM05 04-n225, a schema change that alters what a card *does* or how it is *strategically classified* resets that card's `design_pass` to 0 here **and** blanks its `.md` Status table row; a change that merely populates a derived field does not. Applied at S157 (7 cards) and S159 (8 cards recategorised to `PositionalWager`). Query the table rather than quoting a count from this file.
...
**⚠ Governing caveat — read before using any table/view in this section.** `base_uvm_cost` rates are *calibrated by averaging the existing designed cost of cards that already use a given (Subject) or (Subject, Function) pair* — they are not derived from playtesting, simulation, or any external ground truth. "Validated" (`confidence_tier`) means "backed by ≥2 existing card cost samples that agreed closely enough to average," not "confirmed correct by play." Sample sizes are thin corpus-wide: `uvm_assumptions` is 7 validated / 4 single-example / 17 fallback-avg (of **27** rows as of S157); `uvm_pair_assumptions` is 25 validated / 33 single-example (of **57** rows, zero `fallback_avg` — pairs fall back to `uvm_assumptions` instead, compounding the thinness). **S157 subject-table corrections:** `Difficulty` renamed to **`ModifierToken`** in both tables (a pure relabel preserving the 2.50 pair rate — schema_cleanup #27 reclassified DIR.CA.8's subject at S149 but never updated the pricing tables, silently dropping the card out of the cost model until S157); `NamedActionType` **removed** from both after SYN.CA.5's rescope retired the subject. `bash tools/sync_card_db.sh` now reports any Subject in use with no base rate — that failure mode is caught automatically rather than found by accident. **Any `value_rating`, cost proposal, delta, or tier derived from these tables inherits this assumption chain.** Treat every number from this system as a self-consistency check against the existing corpus, not an external validity check — it can catch a card that's out of line with its siblings, it cannot tell you the sibling group itself is priced right. Re-derive/re-calibrate once real playtest data exists (session count, win-rate correlation, or subjective "this felt broken" reports against actual cost).
...
- **`card_effect_component`** (S143; **rebuilt S157, 355 rows**) — effect-tier mutations per card. **Extractor: `tools/extract_card_effects.py` (added S157, wired into `tools/sync_card_db.sh`).** Until then this table had *no* extractor at all — populated once by an ad-hoc script that was not kept, and by S157 32% of its rows still used the `+=`/`-=` syntax swept out of the corpus months earlier, so every `value_rating` in the corpus was derived from a stale snapshot. **It also reads `on_accept`/`on_decline`/`persistence_effect`**, which carry the entire effect on ElectPlayer and card-as-condition cards; the `tier` ENUM cannot hold those names, so they are stored as the tier they resolve at with the source field prefixed onto `raw_expr` (e.g. `on_accept: ...`). Omitting them prices those cards at 0.00. **Resolved S158 (04-n228, Andy): `persistence_effect` counts toward cost.** Not an inference — the pre-rebuild backup carries five rows literally prefixed `[persistence_effect]` (DIR.PA.3/5/6/11, DIR.MOD.9, all `board_condition`/NULL), and the existing S145 ratings on those cards reconcile only with inclusion. Whether it is encoded as `board_condition`/NULL (old) or as a concrete category + magnitude (current) is tier-neutral corpus-wide. The S157 claim that this choice alone moved SYN.MOD.6 from tier 1 to tier 4 does **not** hold: SYN.MOD.6 computes tier 4 either way, and as a MOD card it is not UVM-tiered at all. The choice gated exactly one card, SYN.PA.3. `card_id VARCHAR(15)`, `tier ENUM('success','successcrit','fail','failcrit')`, `category ENUM('presence_delta','standing_delta','resource_delta','token_delta','board_condition','accord_action','reveal','other')`, `target ENUM('acting','target','third_party','district','none','other')` — **rewritten S160 (04-n234): now names THE ENTITY WHOSE HOLDINGS THE ROW CHANGES** (it previously named whichever entity a regex cascade mentioned first, and disagreed with this on 108 of 355 rows) — `magnitude INT NULL` (NULL = play-determined/declared-N, not a data gap), `raw_expr TEXT`. Feeds `v_card_pair_uvm_cost`.
...
### View DDL — where it lives (S160)
Until S160 **no view in this chain was defined by any file on disk** — they existed only in the live DB, so a rebuild would have silently lost the cost model. Two are now versioned and are the source of truth for their definitions; edit the file, then load it:
- `Database/view_card_pair_uvm_cost.sql` — the working view (carries the 04-n229 fix)
- `Database/view_card_value_to_acting.sql` — the beneficiary axis (not wired into pricing)

The remaining views in the chain are still live-DB-only and should be captured the same way when next touched.
...
~~**Self-cost-vs-delivered-value confusion**~~ — **CLOSED S160 (L373 + L374 + L376).** The axis exists: `card_effect_component.target` names the entity whose holdings a row changes, and `v_card_value_to_acting` derives `magnitude × (+1 if acting else −1)` from it. It is **deliberately not wired into `v_card_pair_uvm_cost`.** Andy's ruling (L376) is that `value_rating` means **gross effect delivered** — how much of the game a card moves — not a net ledger; what the acting faction pays is already recorded by `cost`. A signed measure was wired in briefly and reverted: it rated pay-to-impose cards as floor-tier for paying a fair price. If you are tempted to rewire it, read the header of `Database/view_card_pair_uvm_cost.sql` first.

**`target` was wrong on 108 of 355 rows until S160 — FIXED (L373).** `find_target()` was a four-rule first-match-wins cascade answering "which entity is textually mentioned, by rule precedence." All four defect classes are fixed: hardcoded `faction(Syndicate)`-style names now resolve against the card's own faction (`acting` on their own card, `third_party` on another's); the `district` test no longer pre-empts `faction=`; type/currency qualifiers (`IntelToken(faction=X)`, `native(faction=X)`, `source=`) are blanked before the receiver search, with the accessor preserved; and transfers get a destination rule so a single-row transfer says whether acting received or gave. Distribution moved 106→221 `acting`, 121→90 `target`, 73→17 `other`, 55→16 `district`, plus 7 `none` and 4 `third_party`. The residual 17 `other` are genuinely unresolvable from the expression alone and are labelled so rather than guessed.

~~**Redirect double-count (04-n229)**~~ — **CLOSED S160 (L372).** An `xfer` CTE in `v_card_pair_uvm_cost` collapses matched opposing legs on a Redirect card into one Redirect unit, which fixes the double-count and makes the `board_condition` and `presence_delta` paths converge on the same rate. Exactly 3 of 205 cards moved (GD-01 22.00→12.00, GHO.MOD.7 and NET.MOD.10 5.44→1.67); all three are ModReactCards, so no CA/PA `value_rating` moved. `StructureBlock/Redirect = 10.00` remains n=1.

**NULL-magnitude fallback** misattributes unrelated effect rows onto a card's single declared pair on multi-effect/scaling cards.

**Mixed quantification across the ledger (04-n235, S160)** — when some of a card's effect rows carry a readable magnitude and others do not, and they sit on opposite sides of the acting / non-acting split, whichever side is unreadable is floored at 1 unit while the other counts in full. Runs in **both** directions: downward (SYN.PA.1's `count=n` swing, GUI.PA.2's AccordForm, STD.CA.9's Overture) and upward (GUI.PA.6's given-away structure floored against a counted 3-native gain, so its 8.50 overstates it). Holds 5 ratings. Note this is **not** the same as NULL magnitude in general: of 101 NULL-magnitude rows across 84 cards, most price *correctly* at 1 unit, being a fixed single thing or not a quantity at all. `find_magnitude` was extended S160 to read `.native * N`, `IntelToken(...) * N` and `min(N, ...)`; it deliberately does not read `structure * 1` or `trigger.amount * 2`, where the multiplier applies to a counted thing.

~~**Net ledger vs power tier (04-n236, S160)**~~ — **CLOSED by ruling (L376): the tier is gross effect delivered.** Recorded as governing in Art 04c (relocated from Art 00c §5 at S162), which had never stated what `value_rating` means — the omission is what allowed the net-ledger measure to be wired in at all. Under gross the CA/PA corpus reads **zero drift**.

Full detail and worked examples: PM05 04-n229/04-n234, PM02 L370/L371. (The former pointer here, `Whiteboard/cost_baseline_recommendations.md` §4, was stale — that file was retired to `Retired/Whiteboard_Archive/` at S153, PM02 L347.)
...
- `card_subject_map`: **27 rows** (22 mapped to component_id; 5 NULL for non-component subjects). `BattlefieldStrength` added S163 with `component_id = NULL` — a contesting faction's Battlefield Strength total (Art 03 §10.1.2) is a derived quantity with no physical piece, same treatment as `InfluenceTier`. The table is **DB-resident and not rebuilt by `sync_card_db.sh`** — there is no load file, so a row added here survives syncs but is not in git. Add rows with the INSERT pattern documented in the audit section.
