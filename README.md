...
**Design milestone:** S156 — 09-16 steps 4–5 complete (the primary Art 04 sign-off gate). New `card_body`/`card_restriction_clause`/`v_card_body` EAV tables in `the_signal_db` mirror every Card() field, re-syncable via `tools/extract_card_body.py`. All 5 faction re-audits (STD+faction) + cross-faction synthesis in one session. §9.2 re-sorts to 3 buckets; Ghost = intel-hub; modifier corpus closed most baseline gaps; 5 distinct control poles. §5a Network rewritten (Tripwire retired as stale). PM05 09-17 consolidated triage. Art 03 v4.15 / Art 04 v0.9.95. PM02 L356–L357. Prior (S155): Sequencing locked: PM05 09-16 steps 4–5 confirmed primary; new card_checklist DB table; 18 card headings fixed; Backdate/Field Verification relocated. PM02 L352–L354. Prior (S154): MILESTONE — full 385-card corpus Design Pass; card_status DB fully synced. PM02 L350–L351.
...
| # | Artifact | Ver | Status |
|---|----------|-----|--------|
| 04 | [Card Set: Action Subroutines](V1/04___Card_System___Part1_Core.md) | 0.9.95 | 🔄 In progress — S156: PM05 09-16 steps 4–5 COMPLETE (the primary Art 04 sign-off gate, PM02 L357). New `card_body`/`card_restriction_clause`/`v_card_body` EAV tables + pivot view in the_signal_db, re-syncable via `tools/extract_card_body.py` + `Database/card_body_schema.sql`. All 5 faction re-audits (STD+faction) + cross-faction synthesis; docs in `Whiteboard/{faction}_reaudit_S156.md`, `cross_faction_synthesis_S156.md`, `audit_findings_log_S156.md` (22 open findings → PM05 09-17 consolidated triage). §9.2 re-sorts to 3 buckets (mono {DIR,SYN}/native-cross {GUI}/foreign-gated cross {GHO,NET}), correcting baseline mono grouping; Ghost = intel-hub for NET+SYN+DIR; modifier corpus closed most baseline gaps; 5 distinct control poles. §5a Network rewritten (Tripwire retired), monolith regenerated. Ghost CA.11 flagged for full reimagining (does not gate Art 04). Three scope corrections locked as audit ground rules; two open items (Network/Ghost). PM02 L356–L357. Prior — S155: Sequencing locked (PM02 L352): PM05 09-16 steps 4–5 confirmed primary over the 95-card Art 03 procedure-gap list (04-n221). New `card_checklist` DB table (PM02 L353, 6,970 rows/383 cards). Fixed 18 cards' headings; relocated Backdate/Field Verification to §15. Steps 4–5 scope refined — full CA+PA+MOD per faction, plus new implied-vs-leverageable-strategy dimension. PM02 L352–L354. Prior — S154: MILESTONE — full 385-card corpus Design Pass; 4,685 boilerplate comments stripped; 95-card Art 03 procedure-gap backlog (04-n221) identified; card_status DB fully synced. PM02 L350–L351. |
