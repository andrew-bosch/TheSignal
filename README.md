...
**Design milestone:** Session 163 — Art 04 §6.6 Canonical Card Blocks added and schema scaffolding swept corpus-wide (04-n177 closed); Art 03 v4.16 adds §18.2 Pay Cost and §13.6 Intel Token as Cost, closing 04-n227 and the subclass-wide half of 04-n221.
...
*   **Art 04 Schema Scaffolding:** `python3 tools/scaffold_n177.py` (dry-run by default; `--write` to apply). Inserts any §6.1 field a `Card()` block omits, per the S163 rule that every field is explicit even at its default (PM02 L381). Depth-aware — it anchors to the line a field's *value ends on*, never a line inside a multi-line expression. Its `SKIP_FIELDS` set encodes S163 rulings, not permanent truth: `is_unique`/`deck_limit` are skipped only while blocked on PM05 04-n136 (see 04-n238), and `acquisition`/`generating_card` are skipped because §6.2 gives them an omit-unless-`Issued` default. Re-run it after adding cards to confirm the corpus still reports zero omissions.
