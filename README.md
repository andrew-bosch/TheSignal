...
**Design milestone:** S159 — schema_cleanup_log #64 and #65 closed; every Art 04 schema gate clear; next is card-audit and content work. `persistence_effect` counts toward `total_pair_cost` (evidenced, not inferred: the pre-rebuild backup carries `[persistence_effect]`-prefixed rows and the S145 ratings reconcile only with inclusion). 10 CA/PA cards re-rated against the rebuilt model; `sync_card_db.sh` reports ✓ no drift. Three S157 claims overturned on re-derivation — SYN.MOD.6's tier never depended on the choice, there are five bare-prose PAs not four (NET.PA.6 found), and drift is 16 not 14. The blanket provisional-`value_rating` caveat is lifted except for the five bare-prose holds. Art 04 v0.9.97. Prior — S157: PM05 09-17 triage complete (the second and last original Art 04 sign-off gate). All 27 S156 audit findings triaged; every `[verify]` finding re-derived against primary source, overturning four conclusions and correcting five claims. §9.2 grouping fails against corpus data (cross-cost share flat); 04-n223 scoped to re-derive; SYS-1 reframed as symptom of undesigned sub-6-player area → 00a-80 (explicitly deferred). Schema cleanup #59–#63/#66 closed: `resolution_type` populated, two `value_rating` conventions documented, SYN.CA.5 rescoped v2.0, NET.MOD.3 boost mechanic, GUI.CA.2/GUI.CA.6 payout defect fixed, `card_effect_component` extractor built + synced, 18 cards change tier (pending 04-n228). New `tools/sync_card_db.sh` one-command rebuild + sync all DB mirrors. Audit archived to `Whiteboard/last_full_card_set_audit.md`. Design Pass materiality test confirmed (04-n225). Art 03 v4.15 / Art 04 v0.9.96. PM02 L358–L366. Prior (S156): Primary sign-off gate complete; new card_body/card_restriction_clause/v_card_body EAV tables; all 5 faction re-audits + cross-faction synthesis; §9.2 3-bucket sort; Ghost intel-hub; modifier corpus closed gaps; 5 control poles; §5a Network rewritten; PM05 09-17 consolidated triage. PM02 L356–L357. Prior (S155): Sequencing locked; PM05 09-16 steps 4–5 primary; card_checklist DB table; 18 card headings fixed; Backdate/Field Verification relocated. PM02 L352–L354. Prior (S154): MILESTONE — full 385-card Design Pass; card_status DB fully synced. PM02 L350–L351.
...
| 00c | [Economy Manifest](V1/00c___Economy_Manifest.md) | 0.6 | 🔄 In progress |
...
| # | Artifact | Ver | Status |
|---|----------|-----|--------|
| 04 | [Card Set: Action Subroutines](V1/04___Card_System___Part1_Core.md) | 0.9.99 | 🔄 In progress — S159: schema_cleanup_log #64 and #65 CLOSED (PositionalWager re-derived corpus-wide 13→21; §6 swept to spec) — `persistence_effect` counts toward cost; 10 CA/PA cards re-rated; drift now clean; five bare-prose PAs hold pending 04-n218/n220; three S157 claims overturned on re-derivation. PM02 L367. Prior — S157: PM05 09-17 triage COMPLETE (the second and last original Art 04 sign-off gate, PM02 L358). All 27 S156 audit findings triaged; every `[verify]` finding re-derived against primary source, overturning four conclusions and correcting five claims. §9.2 grouping fails corpus data (flat cross-cost share); 04-n223 re-derive grouping; SYS-1 reframed as undesigned sub-6-player symptom → 00a-80 (explicitly deferred from gating Art 04). Schema cleanup #59–#63/#66 closed: `resolution_type` populated on 46 cards; #60 two `value_rating` conventions coexisting (CA/PA vs modifier), 9 cards re-tiered; #61 SYN.CA.5 rescoped v2.0 (`faction != acting` self-exemption); #62 NET.MOD.3 boost mechanic → 26 cards' Art 03 §18 cost-payment gap folded into 04-n221; #63 GUI.CA.2/GUI.CA.6 live payout defect fixed; §6.1 `PortraitEntry.flat` removed. **#66 critical:** `card_effect_component` (substrate of every `value_rating`) had no extractor, month stale; built `tools/extract_card_effects.py`, rebuilt table, 18 of 205 cards change tier (none applied pending 04-n228). New `tools/sync_card_db.sh` regenerates monolith + rebuilds all DB mirrors in one command. Audit archived to `Whiteboard/last_full_card_set_audit.md`. Design Pass materiality test confirmed (04-n225). PM02 L358–L366. Prior — S156: Primary sign-off gate complete (PM02 L357): PM05 09-16 steps 4–5 COMPLETE; new card_body/card_restriction_clause/v_card_body EAV tables; all 5 faction re-audits + cross-faction synthesis; §9.2 3-bucket sort; Ghost intel-hub; modifier corpus closed gaps; 5 control poles; §5a Network rewritten; Ghost CA.11 flagged full reimagining. Prior — S155: Sequencing locked (PM02 L352): steps 4–5 primary; new card_checklist DB; 18 headings fixed; Backdate/Field Verification relocated. |
