...
**Design milestone:** S143 — Floor Act designed (STD.PA.9 Town Hall, PM02 D04-13/L216); new `on_discard` field/Principle P29; `value_rating` moved to base Card() class + scaffolded across 251 CA/PA specs; GD-01 Grant Deed redesigned v0.3→v0.4. Major UVM (Universal Value Metric) pricing pipeline built in DB feeding the value_rating→cost mapping — full detail in PM05 04-n178. Outlier review surfaced concrete redesign needs — see `Whiteboard/cost_baseline_recommendations.md`. Next: work the outlier/redesign-needs checklist, then bucket total_pair_cost into the 1–4 value_rating tiers. Prior (S142): 09-16 step 2 PA design review complete (45 cards, all 6 sets). Prior (S141): CA design review complete (69 cards). Prior (S140): ModBattleCard review complete (44 cards). Prior (S139): ModActionCard review complete (132 cards). Prior (S138): ModReactCard review complete (81 cards). Prior (S136): Art 04 physical split into 8 Part files.
...
| # | Artifact | Ver | Status |
|---|----------|-----|--------|
| 04 | [Card Set: Action Subroutines](V1/04___Card_System___Part1_Core.md) | 0.9.86 | 🔄 In progress — S143: Floor Act designed (STD.PA.9 Town Hall); new `on_discard` field/Principle P29; `value_rating` moved to base Card() class + scaffolded across 251 specs; GD-01 redesigned v0.3→v0.4; major UVM pricing pipeline built in DB (uvm_assumptions, uvm_pair_assumptions, v_card_pair_uvm_cost) feeding value_rating→cost mapping — full detail in PM05 04-n178; outlier review surfaced concrete redesign needs (see `Whiteboard/cost_baseline_recommendations.md`). Next: work the outlier/redesign-needs checklist, then bucket total_pair_cost into 1–4 value_rating tiers. Prior — S142: PA review complete (45 cards). Prior — S141: CA review complete (69 cards). Prior — S140: ModBattleCard review complete (44 cards). Prior — S139: ModActionCard review complete (132 cards). Prior — S138: ModReactCard review complete (81 cards). |
