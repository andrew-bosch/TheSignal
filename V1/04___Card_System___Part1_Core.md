...
| §15 | [Deferred Design — Blocked Cards](#15-deferred-design--blocked-cards) |
...
| value_rating | Metadata | int \| None | 1–4. Power/strength tier printed on card face; used in Splay calculation for Modifier cards. Base Card() field — all card types inherit it. Assigned corpus-wide via the UVM pair-pricing model — derivation methodology, governing caveat, and tier boundaries: Art 04c. `None` = unassigned. | Face |
...
| GD-01 | Grant Deed | 📝 | Territory | Public | Redirect | Structure Block | Move | *ARBITER-issued ModReactCard (`acquisition=Issued`), generated by SYN.CA.8 and GUI.CA.10 — not deck-drawn. Carries real taxonomy; see §12b.*
...
4. **Only if step 3 executed:** remove 1 Structure Block belonging to the triggering faction from `deed.district` — the registered deed takes precedence over the unauthorized build that just fired it. If GR 8.2 skipped step 3, step 4 is skipped with it: the deed entitles the holder to build, and where the holder has already built, the claim is satisfied and there is nothing left to enforce against.
...
GD01 = Card(
    ...
    success = [arbiter.place(presence_chip, district=deed.district, faction=holding, count=1),
               if district(deed.district).faction(holding).structure == 0: (
    ...
    narrative    = None,
    perspectives = None,
    design_note  = "ARBITER-issued (acquisition=Issued); not drawn from a deck. Fill-in fields: district (from generating CA target) and holder (acting faction of generating CA). GR 8.2 governs step 3 — structure block placement blocked if holder already holds one in deed.district; step 2 (Presence Token) always executes on fire. Step 4 is gated on step 3 actually executing: when GR 8.2 blocks the placement, the removal is skipped too, so the card never resolves as a bare removal. Multiple deeds on the same district are permitted; each fires independently. Produced by SYN.CA.8 Land Title and GUI.CA.10 Development Order. Step 4 (removing the triggering faction's structure block) reflects that a registered claim doesn't just let the holder catch up when someone else builds — it displaces that build outright: a Land Title/Development Order registered earlier legally supersedes an unauthorized structure raised later on the same ground. This interacts with SYN.CA.8/GUI.CA.10's own cost calibration.",
...
## 15. Deferred Design — Blocked Cards

Cards that have been removed from the active §7 corpus — they are not in play, not part of any faction's deck, and out of scope for the standard 17-row Design Checklist review — pending a fundamental redesign that hasn't happened yet. Both entries below carry the retired `id="Ghost-ext-TBD"` placeholder rather than a real `FAC.TYPE.n` card_id (ID-04 renumber never assigned one, since assigning a real ID to a permanently-blocked mechanism was judged premature — the redesign may change what the card does enough to change its taxonomy). Relocated here S155 (moved out of Ghost §7, Andy's direction) so §7 only holds cards that are actually part of the live corpus; content is unchanged from its last edit. Open tracking: PM05 04-n103 (original block ruling, S107/closed) and 04-n205 (open — cost-typing note, explicitly deferred to whenever this redesign happens).

### Backdate 🚫 BLOCKED — Deferred Design
[↑ 15. Deferred Design](#15-deferred-design--blocked-cards)
...
### Field Verification 🚫 BLOCKED — Deferred Design
[↑ 15. Deferred Design](#15-deferred-design--blocked-cards)
