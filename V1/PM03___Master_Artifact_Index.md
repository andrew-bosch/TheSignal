...
| 03a | Game Engine Specification | 0.1 | 🔄 Draft — Placeholder | Code-lite technical companion to Art 03. Three layers: (1) State Model — formal game state at each beat boundary using 00b entity IDs; (2) Beat Procedures as pseudocode — Beat_0()–Beat_5() with IF/THEN/ELSE branches, modifier stack as summation formula, resolution check as formal inequality; (3) Decision Tables & Edge Case Registry — 9 tables covering face-down/face-up, Apex, Critical overrides, partial payment, Infrastructure (L107), Type B scope, Emergency Response assist/thwart. Variable modifier rows M-08/M-09/M-10/M-12 stubbed pending Art 04. Established session 20. |
...
| PM02 | Decision Log & Validation Tracker | 1.9 | 🔄 Active | Locked decisions (L01–L109), open blocking decisions, playtest variables. §2b punch list archived — live version in PM05. Change log through session 20. |
| PM03 | Master Artifact Index (this document) | 1.6 | 🔄 Active | Artifact registry, standard artifact template, dependency map, retired artifacts index (/Retired/). Design standards and conventions moved to PM04 §2. |
...
| PM05 | Active Punch List | 1.5 | 🔄 Active | Living action queue of all pending changes across all artifacts. Session 20: XA-24 closed (L107), XA-25/26/27/29 added, PM04-03/04 added, Art 03 sign-off. |
