...
| NS-xx | Notification Slip | Pre-printed ARBITER slip. Content: "A covert operation was conducted against your interests this round." No faction identified, no operation type. Dropped into target faction's dispatch case on covert op fail conditions only. One standard text — not written per-incident. | Art 04, Art 07 | ⬜ Schema pending — S68 definition |
| IS-xx | Intel Delivery Slip | ARBITER-written per-incident intelligence note delivered to acting faction's dispatch case on card success conditions. Content varies by card procedure: may contain a single operation's details or a list of operations. Delivered at Beat 2 or Beat 3 resolution as specified by the card's procedure. Not a pre-printed form — ARBITER writes content at resolution time from data already present in the resolution grid. Not proactive tracking — ARBITER executes card procedure only. Known uses: C17 (Beat 3, single op — op type + target district); C18 (Beat 2, faction column — op name + declared target per op); C24 (Beat 2, district row — op type per op). | Art 04, Art 07 | ⬜ Schema pending — S68 reinstatement |
| VM-xx | Visibility Marker | ARBITER-held transient token. Placed on a Beat 3 grid card when that operation has been publicly revealed at Beat 2. At Beat 3: signals ARBITER to resolve that card publicly — announce name, type, and declared targets before rolling; announce d100 result and outcome aloud to all players after rolling. Removed to ARBITER supply immediately after that card's Step 8 cleanup. Never placed on the Overview. Physical form: single distinctive token in ARBITER supply. | Art 04 C28; Art 03 §11 | ⬜ Schema pending — S81 registration |
| BM-xx | BoostMarker | ARBITER-held transient token. Placed on a card's grid slot at Beat 0 when excess boost payment is detected — n tokens placed = boost count n. Covert operations only. At Beat 2/3: n BM-xx present signals that the outcome effect executes (1+n) times (success effects only — fail and failcrit fire once regardless). Removed to ARBITER supply after effect execution at beat cleanup. Physical form: token marked "Boost." | Art 03 §11; Art 04 | ⬜ Schema pending — S79 draft |

*24 entity types. IS-xx (Intel Delivery Slip) removed S68 then reinstated S68 — removal was a misdiagnosis (R40 does not prohibit card-procedure-driven delivery). Definition expanded S68 to cover Beat 2 and Beat 3 delivery, column and row reads. DR-xx (Dispatch Report) collapsed into IS-xx S68 — DispatchReport was a specialised IS-xx; generalised delivery mechanism covers all cases. C17 spec review pending (PM05 04-n39). 9 fully L108-compliant. 3 with existing schemas pending further work (PB-xx: ranges pending; C-xx + P-xx: L108 audit pending XA-28). 9 schemas pending source artifact design. 1 cross-artifact standard defined here (VS-xx).*
