...
| IS-xx | Intel Delivery Slip | ARBITER-written per-incident note delivered to acting faction's dispatch case on card success conditions. Content specified by the resolving card's success procedure (e.g., C17: op type + target district of intercepted operation). Not a pre-printed form — content varies by card. Written at Beat 3 resolution from Beat 0 grid data. Not proactive tracking — ARBITER executes card procedure only. | Art 04, Art 07 | ⬜ Schema pending — S68 reinstatement |
| DR-xx | Dispatch Report | ARBITER-prepared per-round intelligence report delivered to acting faction's dispatch case before Beat 3 resolution. Fields: faction (target) + operations[] containing name and declared target (district/faction/object) for each op in target faction's Beat 3 resolution grid. Modifier cards excluded (splayed edge only, not readable). Generated from Beat 0 grid data. Delivered at beat3_pre_resolution timing. Currently used by: C18 Dossier Breach. | Art 04, Art 07 | ⬜ Schema pending — S68 |

*23 entity types. IS-xx (Intel Delivery Slip) removed S68 then reinstated S68 — removal was a misdiagnosis (R40 does not prohibit card-procedure-driven delivery; Beat 0 grid construction means ARBITER already holds op type + district data when C17 fires). Definition tightened on reinstatement (see IS-xx row). C17 spec review pending (PM05 04-n39). 9 fully L108-compliant. 3 with existing schemas pending further work (PB-xx: ranges pending; C-xx + P-xx: L108 audit pending XA-28). 9 schemas pending source artifact design. 1 cross-artifact standard defined here (VS-xx).*
