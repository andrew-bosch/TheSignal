...
| XA-27 | **03a — Modifier & Threshold Reference supplement.** Once Artifact 04 card designs are locked (all modifier values confirmed), populate 03a as the exhaustive modifier catalog and balance analysis document. **Taxonomy structure is now defined** — §14's 7-column table (Category / Modifier / Scope / Applied / Instance Limit / Value Type / Threshold Adjustment) is the data schema for 03a. 03a expands this with: (1) all Variable modifier values from cards once Art 04 is locked; (2) worst-case stack analysis per action type; (3) fairness audit — positive modifier ceiling vs. negative modifier floor; (4) faction distribution — which categories are available to which factions and when. **Known worst-case stack** (all Fixed modifiers): partial payment −50 + Infrastructure −25 + Discredited −20 + Type B −15 = −110 (any difficulty tier reduced to Critical Success only). Variable modifier stack unknown pending Art 04. Begin after Art 04 C01–C35 and P01–P18 are locked. | Material (new artifact) | Session 20 modifier audit | 🔄 |
