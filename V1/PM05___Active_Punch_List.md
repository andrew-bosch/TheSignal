...
| XA-30 | **Compound effect text — flag as known L108 gap in Artifact 00b §8.** L108 Requirement 1 (no compound cells) is violated by multi-effect prose strings in all Effect on success fields (e.g., C01 places a structure AND generates upkeep resource — two distinct effects in one text string). The taxonomy triple (Category/Function/Target) is the correct decomposition model — but decomposing effect fields into typed sub-components (effect type, quantity, target, condition) requires content to be locked first. Action: add a flagged entry to 00b §8 (Design Notes & Known Gaps) documenting this as a known L108 violation, deferred until card content is locked. Non-material — no mechanics change. | Non-material (gap flag) | Session 23 research, L108 | 🔄 |
