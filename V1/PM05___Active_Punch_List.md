...
| # | Change | Type | Source | Status |
|---|--------|------|--------|--------|
| XA-22 | **Resolution Grid — design and produce physical component.** Staging grid used by the ARBITER Player to sort all dispatch case contents before Beat 2 processing begins. Structure: 5 lanes (columns) by case receipt order; rows for Beat 2 cards, then paired Beat 3 card/target rows (up to 4 pairs for Ghost). Beat 2 resolves left to right. Beat 3 resolves row-first: all card-1 pairs across all lanes before any card-2 pair (round-robin by case receipt order — L102). Beat 4 not included (public declarations, initiative order). Mixed case contents resolved in ARBITER's placement order. **Modifier card cascade:** modifier cards stack under the operation card in each grid cell, peeking out at top/bottom to display values at a glance — card physical design must support this orientation (value printed at both top and bottom edge). Design in Artifact 07 (ARBITER workflow + component spec); physical production in Artifact 11; add to PM01 component list. Reference added to Artifact 03 Beat 2 and Beat 3 (session 16). | Material (component design) | L102, session 16 | 🔄 |
| XA-21 | **ARBITER visual indicator for active Situation Report targeting restrictions.** During Beat 3, ARBITER checks whether a district or ring has an active targeting restriction announced in Beat 1. This check is distinct from the deployment marker Blocked face (which signals conversion block only). ARBITER needs a physical visual indicator — options: restriction overlay card placed on affected districts or rings during Beat 1 announcement, dedicated restriction marker type, or color-coded indicator on The Overview. Must be visually distinct from the Blocked face marker. Design in Artifact 11; procedure in Artifact 07 (ARBITER workflow); component listed in Artifact 01 (board component spec) and PM01. Targeting restrictions and conversion blocks are intentionally separate effects — a Situation Report may carry either or both independently. | Material (component design) | Session 16 | 🔄 |
