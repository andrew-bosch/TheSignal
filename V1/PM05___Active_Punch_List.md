...
| 04-25 | **Resolution type field — classify all cards and define enum after full card set.** New §6 schema field added (working name: Resolution type). Stub only — field definition in place, all current cards carry N/A pending classification. Once C01–C35 and P01–P18 are all designed: (1) define the final enum values (known candidates: Transactional / Probabilistic / Positional — may expand as full card set reveals new patterns); (2) finalize field name; (3) back-fill Resolution type into all card entries; (4) this classification then feeds 00c §8 Derived Cost Analysis (separate modeling formula per resolution type) and a future resolution-type taxonomy document (home TBD alongside 04b). Not a blocker for C11–C15 sign-off or Art 04 completion — schema field is additive. | Non-material (schema addition) | Session 30, C11 discussion | 🔄 |
