...
| 02a-WBS-01 | **Add Established marker row to PM01 WBS 2.** New component added session 22 (L111): silver marker, one per Established faction per district, quantity TBD pending Art 11 component spec. Add as new WBS 2 row after 2.03 (deployment marker). | Non-material (component list) | L111 | 🔄 |
...
| XA-27 | **03a — Game Engine Specification.** Scope expanded session 20: from modifier catalog to full code-lite technical companion to Art 03. Three layers: (1) **State Model** — formal game state at each beat boundary using 00b entity IDs (M-xx, D-xx, RO-xx) as variable vocabulary; (2) **Beat Procedures as pseudocode** — each beat as a structured function (Beat_0() through Beat_5()) with explicit IF/THEN/ELSE branches, named inputs/outputs; modifier stack as summation formula; resolution check as formal inequality; (3) **Decision Tables & Edge Case Registry** — all branching conditions surfaced as tables (face-down/face-up, Apex vs. non-Apex, Critical overrides, partial payment, Infrastructure, Type B scope). Variable modifier rows (M-08 Modifier card, M-09 Protect, M-10 Situation Report) stubbed pending Art 04. Fixed modifier math fully specifiable now. Modifier balance analysis (original XA-27 scope) is a derived output once formal stack is expressed. **Placeholder created; structure defined; content pass queued for near-term session.** | Material (new artifact) | Session 20 | 🔄 |
